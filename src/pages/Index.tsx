  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <section
        ...
        className={`py-16 sm:py-20 md:py-32 transition-all duration-700 ${solutionsSection.isVisible ? 'animate-fadeInUp opacity-100' : 'opacity-0'
          }`}
      >
      ...
      {/* Manifesto Teaser Section */}
      <div className="text-center max-w-4xl mx-auto"> {/* დაემატა text-center და max-w */}
        <h2 className="text-3xl sm:text-4xl md:text-5xl font-bold mb-6 sm:mb-8 leading-tight text-slate-900 dark:text-white">
        ...
        <div className="space-y-4 sm:space-y-6 text-base sm:text-lg text-slate-600 dark:text-slate-300 mb-8 sm:mb-10"> {/* ფერები შესწორდა */}
          <p>
            While others sell{' '}
            <span className="text-slate-900 dark:text-white font-medium">complexity</span> {/* text-white შეიცვალა */}
            {' '}and{' '}
            <span className="text-slate-900 dark:text-white font-medium">buzzwords</span>, {/* text-white შეიცვალა */}
            {' '}we offer{' '}
            <span className="text-emerald-600 dark:text-emerald-400 font-medium">mathematics</span> that works.
          </p>
          <p className="text-sm sm:text-base text-slate-500 dark:text-slate-400 font-medium"> {/* ფერი შესწორდა */}
            N → N Economics: Making counterfeiting mathematically unprofitable.
          </p>
        </div>

        <div className="flex flex-col sm:flex-row gap-4 justify-center">
          <Link to="/manifesto">
            <Button size="lg" variant="outline" className="w-full sm:w-auto border-slate-200 dark:border-slate-700 text-slate-900 dark:text-white hover:bg-slate-100 dark:hover:bg-slate-800">
              <FileText className="mr-2 h-5 w-5" />
              Read Full Manifesto
            </Button>
          </Link>
          <Link to="/register">
            <Button size="lg" className="w-full sm:w-auto"> {/* ნაგულისხმევი primary ღილაკი უკეთესია აქ */}
              Get Started Free
        ...
        {/* Quote */}
        <div className="mt-12 sm:mt-16 p-4 sm:p-6 rounded-xl bg-slate-100 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 max-w-2xl mx-auto">
          <blockquote className="text-sm sm:text-base text-slate-700 dark:text-slate-300 italic"> {/* ფერი შესწორდა */}
            "They sell complexity because complexity is expensive.
            <br />
            We offer simplicity because simplicity works."
          </blockquote>
        </div>
      </div>
      ...
      >
        <div className="container mx-auto px-4 max-w-6xl relative">
          {/* Blurred Content */}
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-6 sm:gap-8 blur-sm select-none opacity-50">
